	...
	<aui:row>
		<aui:col cssClass="lfr-asset-column-details" width="<%= 70 %>">
			...
			<div class="body-row">
				...
				<div style="height:50px !important;float:left;">
	                <liferay-ui:ratings
					            className="<%= DLFileEntryConstants.getClassName() %>"
					            classPK="<%= fileEntryId %>"
					            type="thumbs"
				...
				<c:if test="<%= showAssetMetadata && PropsValues.DL_FILE_ENTRY_COMMENTS_ENABLED %>">
					<liferay-ui:panel defaultState="closed" collapsible="<%= true %>" cssClass="lfr-document-library-comments" extended="<%= true %>" persistState="<%= true %>" title="comments">
			...
			<div class="body-row asset-details">
				...
				<div class="lfr-asset-panels">
					<liferay-ui:panel-container extended="<%= false %>" id="documentLibraryAssetPanelContainer" persistState="<%= true %>">
						...
						%>
									<!-- add this defaultState="closed" for default closing container -->
									<liferay-ui:panel cssClass="metadata" extended="<%= false %>" id="documentLibraryMetadataPanel" persistState="<%= true %>" title="<%= HtmlUtil.escape(ddmStructure.getName(locale)) %>">
                                         <%
                                         String fieldNamespace =  String.valueOf(ddmStructure.getPrimaryKey()+","+fileEntry.getReadCount());
                                         %>
										<liferay-ddm:html
											classNameId="<%= PortalUtil.getClassNameId(DDMStructure.class) %>"
											classPK="<%= ddmStructure.getPrimaryKey() %>"
											fields="<%= fields %>"
											fieldsNamespace="<%=fieldNamespace%>"
											readOnly="<%= true %>"
						...
						<liferay-ui:custom-attributes-available className="<%= DLFileEntryConstants.getClassName() %>" classPK="<%= fileVersionId %>" editable="<%= false %>">
							<liferay-ui:panel defaultState="closed" collapsible="<%= true %>" cssClass="custom-fields" id="documentLibraryCustomAttributesPanel" persistState="<%= true %>" title="custom-fields">
						...
						%>
									...
									<liferay-ui:panel defaultState="closed" collapsible="<%= true %>" cssClass="lfr-asset-metadata" id="documentLibraryAssetMetadataPanel" persistState="<%= true %>" title="<%= name %>">
						...
						<c:if test="<%= showAssetMetadata %>">
							<liferay-ui:panel defaultState="closed" extended="<%= false %>" cssClass="version-history" id="documentLibraryVersionHistoryPanel" persistState="<%= true %>" title="version-history">
