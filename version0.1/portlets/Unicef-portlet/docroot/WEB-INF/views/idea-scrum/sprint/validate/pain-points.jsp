...
<div class="np-rounded-box clearfix"><div class="chat-container clearfix np-rounded-box">
	<div class="chat-question-box clearfix">
		<div class="chat-user-img-box clearfix">
		    ...
		    <div class="chat-user-img-text06">${inventorName}</div>
	    </div>
	    <div class="chat-user-comment">${sprint6.text}
	    	<p class="sprint-discussion-title-description">${sprint6TitleDescription}</p>
		    <div class="chat-user-reply-section clearfix">
		    	<div class="fr">
		    		<c:if test="${loggedInUserId == inventorId}">
			    		<a href="#" class="chat-user-tnx_btn">Thanks</a>
		    		</c:if>
		    			<a href="javascript:void(0);" class="chat-user-reply_btn" onclick="javascript:<portlet:namespace />postReplay6(0,0,0);">Reply</a>
		    	</div>
			</div>
		</div>
		<div class="chat-video-help "><a class="various-6"  href="#video-button">video link</a></div>
  	</div>
  
     <div id="discussionWrapper6" class="chat-container2">
    ...
    <div id="<portlet:namespace/>replayDiv6" style="display: none;" class="clearfix chat-ans-text-box">
			 <div id='<portlet:namespace/>replay' class="chat-user-img-box clearfix">
    			...
    			<div class="chat-user-img-text06">${loggedInUserName}</div>
    		</div>
			<div id="<portlet:namespace />replayDiv6">
    		<div class="chat-user-comment" id="<portlet:namespace />messageButtonWrapper6">
  				<textarea class="cs-chat-form-control-06" id="<portlet:namespace/>responseDiscussionInput6" name="<portlet:namespace/>responseDiscussionInput6"></textarea>
 				<a href="javascript:void(0);" id='<portlet:namespace/>level-6Comment' class="btn btn-info chat-btn">Reply</a>
 				<a href="javascript:void(0);" id='<portlet:namespace/>level-6Comment-Submit' class="btn btn-info chat-btn np-display-off" onclick="javascript:<portlet:namespace/>sendResponseMessage6();">Submit</a>
 			</div>
			</div>
	</div>
</div></div>
...
<div id="video-button" style="display:none;">
	<div class="responsive-container">
		<iframe allowfullscreen="" frameborder="0" id="player6" mozallowfullscreen="" src="http://player.vimeo.com/video/627329463?autoplay=false&amp;loop=false&amp;byline=false&amp;portrait=false&amp;title=false?autoplay=6" webkitallowfullscreen=""></iframe>
	</div>
</div>
...
$(document).ready(function() {
	...
	<portlet:namespace/>getAllDiscussions6();

	$("#<portlet:namespace/>responseDiscussionInput6").keypress(function (e) {
	        
			if($.trim($("#<portlet:namespace/>responseDiscussionInput6").val()) !== '') {
				$("#<portlet:namespace/>level-6Comment-Submit").removeClass("np-display-off");
			} else {
				$("#<portlet:namespace/>level-6Comment").addClass("np-display-off");
			}
			
			if($.trim($("#<portlet:namespace/>responseDiscussionInput6").val()) === '') { 
				$("#<portlet:namespace/>level-6Comment-Submit").addClass("np-display-off");
				$("#<portlet:namespace/>level-6Comment").removeClass("np-display-off");
			}
		
			var code = (e.keyCode ? e.keyCode : e.which);
	        if (code == 63 && !e.shiftKey) {
	           $("a#<portlet:namespace/>level-6Comment").click();
	        }else{
				return;
			}
	    });
...
<portlet:namespace />autoEnterKey6 = function(ideaScrumDiscussionId){
	$("#<portlet:namespace/>responseDiscussionInput6_"+ideaScrumDiscussionId).keypress(function (e) {
		
		if($.trim($("#<portlet:namespace/>responseDiscussionInput6_"+ideaScrumDiscussionId).val()) !== '') {
...
<portlet:namespace/>getAllDiscussions6 = function(){

	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint6.sprintId}";
	$.ajax({
	    ...
	    success: function( data ) {
			<portlet:namespace/>createDiscussionMessage6(data);
	    },
...
<portlet:namespace/>createDiscussionMessage6 = function(data){
	$("#discussionWrapper6").html(" ");
	var messageHTML = '';
	
	$.each(data, function(index, element) {
		
			messageHTML += '<div id="discussion_'+element.ideaScrumDiscussionId+'" class="Level_'+element.levelCount+'">';	
			messageHTML += <portlet:namespace />generateLevelHTML6(element.ideaScrumDiscussionId, element.avatarURL, element.userName,
			...
			if(element.isChild.toString() == 'true'){
				...
				$.each(element.secondLevelMessages, function(index, secondElement) {
						messageHTML += <portlet:namespace />generateLevelHTML6(secondElement.ideaScrumDiscussionId, secondElement.avatarURL, 
						...
						if(secondElement.isChild.toString() == 'true'){
							...
							$.each(secondElement.thirdLevelMessages, function(index, thirdElement) {
									messageHTML += <portlet:namespace />generateLevelHTML6(thirdElement.ideaScrumDiscussionId, thirdElement.avatarURL, 
			...
			$("#discussionWrapper6").append(messageHTML);
			messageHTML = '';
	});
}
<portlet:namespace />generateLevelHTML6 = function(ideaScrumDiscussionId, avatarURL, userName, message, parentDiscussionId, level, levelCount, isChild,userId,isThanks,isInventorPrefsAdmin,isLiked){
	var levelHTML = '';
	if(isChild == 'true'){
		levelHTML += '<div class="cab-leval-'+levelCount+'" id="cab-level-'+ideaScrumDiscussionId+'">';
						...
						levelHTML +='<div class="chat-user-img-box clearfix">' +
								...
								'</div>' +
								'<div class="chat-user-img-text06">'+userName+'</div>' +
							'</div>' + 
							'<div class="chat-user-ansr-comment"><textarea id="discussion-message_'+ideaScrumDiscussionId+'" class="discussion-message-text cs-chat-form-control-01" readonly="readonly">'+message+'</textarea>'+
								'<div class="chat-user-reply-section clearfix">' +
					            	'<div class="fr">';
					            		...
					            		if(isLiked === false ){
						            		levelHTML += '<a href="javascript:void(0);" class="like-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />deLikeDiscussion6('+ideaScrumDiscussionId+');" id="like-button_'+ideaScrumDiscussionId+'"></a>';
				            				levelHTML += '<a href="javascript:void(0);" class="liked-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />likeDiscussion6('+ideaScrumDiscussionId+');" id="liked-button_'+ideaScrumDiscussionId+'"></a>';
										} else {
											levelHTML += '<a href="javascript:void(0);" class="like-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />deLikeDiscussion6('+ideaScrumDiscussionId+');" id="like-button_'+ideaScrumDiscussionId+'"></a>';
				            				levelHTML += '<a href="javascript:void(0);" class="liked-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />likeDiscussion6('+ideaScrumDiscussionId+');" id="liked-button_'+ideaScrumDiscussionId+'"></a>';													
										}
					            	
					            		if(isInventorPrefsAdmin == "true") {
					            			if($("#<portlet:namespace/>isDiscussionInEditMode").hasClass("np-display-off")) {
					            				levelHTML += '<a href="javascript:void(0);" class="edit-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />editDiscussion6('+ideaScrumDiscussionId+');" id="edit-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="update-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />updateDiscussion6('+ideaScrumDiscussionId+');" id="update-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="cancel-update-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />cancelUpdateDiscussion6('+ideaScrumDiscussionId+');" id="cancel-update-button_'+ideaScrumDiscussionId+'"></a>';
					            			} else {
					            				levelHTML += '<a href="javascript:void(0);" class="edit-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />editDiscussion6('+ideaScrumDiscussionId+');" id="edit-button_'+ideaScrumDiscussionId+'"></a>';	
					            				levelHTML += '<a href="javascript:void(0);" class="update-discussion-board chat-user-tnx_btn np-display-off" onclick="javascript:<portlet:namespace />updateDiscussion6('+ideaScrumDiscussionId+');" id="update-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="cancel-update-discussion-board chat-user-tnx_btn np-display-off" onclick="javascript:<portlet:namespace />cancelUpdateDiscussion6('+ideaScrumDiscussionId+');" id="cancel-update-button_'+ideaScrumDiscussionId+'"></a>';
					            			}							            			
					            		}
					            		
					            		if(isThanks == "false" && isInventorPrefsAdmin == "true"){
					            			levelHTML += '<a href="javascript:void(0);" class="chat-user-tnx_btn" onclick="javascript:<portlet:namespace />discussionThanks6('+ideaScrumDiscussionId+');" id="thanks-button_'+ideaScrumDiscussionId+'">Thanks</a>';
										...
										if(levelCount != 3){
											...
											levelHTML += '<a href="javascript:void(0);" class="chat-user-reply_btn"' +
											                'onclick="javascript:<portlet:namespace />postReplay6('+parentDiscussionId+','+levelCount+','+ideaScrumDiscussionId+');">Reply</a>';
										}
							...
							'<textarea class="cs-chat-form-control-06" id="<portlet:namespace/>responseDiscussionInput6_'+ideaScrumDiscussionId+'" name="<portlet:namespace/>responseDiscussionInput6_'+ideaScrumDiscussionId+'"></textarea>'+
								...
								'onclick="javascript:<portlet:namespace/>sendResponseMessage6();">Submit</a>'+
						'</div>'+
					   '</div>';
		}else{
			levelHTML += '<div class="cab-leval-'+levelCount+'" id="cab-level-'+ideaScrumDiscussionId+'">' ;
...
<portlet:namespace/>deLikeDiscussion6 = function(ideaScrumDiscussionId){
...
<portlet:namespace/>likeDiscussion6 = function(ideaScrumDiscussionId){
	isPopUpEdit=false;
...
<portlet:namespace/>editDiscussion6 = function(ideaScrumDiscussionId){
...
<portlet:namespace/>cancelUpdateDiscussion6 = function(ideaScrumDiscussionId){ 
...
<portlet:namespace/>updateDiscussion6 = function(ideaScrumDiscussionId){ 
	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint6.sprintId}";
...
<portlet:namespace/>discussionThanks6 = function(ideaScrumDiscussionId){
	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint6.sprintId}";
	$.ajax({
...
<portlet:namespace/>sendResponseMessage6 = function(){
	var message = "";
	...
	if(level == 0){
		 message=$("#<portlet:namespace/>responseDiscussionInput6").val();
		  if($.trim(message) == ''){
			$('#<portlet:namespace />messageButtonWrapper6').addClass("has-error");
		  return;
		}
	}else{
		message =$('#<portlet:namespace/>responseDiscussionInput6_'+ideaScrumDiscussionId).val();
		  if($.trim(message) == ''){
	...
	$.ajax({
	    ...
	    success: function( data ) {
			...
			if(level == 0){
				 $('#<portlet:namespace/>responseDiscussionInput6').val('');
				 $('#<portlet:namespace/>responseDiscussionInput6').css('height','54px');
					<portlet:namespace/>goToLastMessage();
					$("#<portlet:namespace/>replayDiv6").toggle(function(){
						$("#<portlet:namespace/>replayDiv6").slideUp();
						isPopUpEdit = true;
					});
				$('#<portlet:namespace />messageButtonWrapper6').removeClass('has-error');
			}else if(level == 6){
				$('#<portlet:namespace/>responseDiscussionInput6_'+ideaScrumDiscussionId).val('');
				 $('#<portlet:namespace/>responseDiscussionInput6_'+ideaScrumDiscussionId).css('height','54px');
...
<portlet:namespace/>goToLastMessage = function(){
	 $("#discussionWrapper6").animate({ scrollTop: $("#discussionWrapper6")[0].scrollHeight}, 0);
 }
 
<portlet:namespace />postReplay6 = function(parentDiscussionId, level, ideaScrumDiscussionId){
	 autosize(document.querySelectorAll('textarea'));
	 <portlet:namespace />autoEnterKey6(ideaScrumDiscussionId);
	if(level == 0){
		...
		$("#<portlet:namespace/>responseDiscussionInput6" ).val('');
		if ($("#<portlet:namespace/>replayDiv6" ).css('display') == 'none') {
			$("#<portlet:namespace/>replayDiv6").toggle(function(){
				   $("#<portlet:namespace/>replayDiv6").slideDown();
				 });
				isPopUpEdit = false;
		}else{
		    $("#<portlet:namespace/>replayDiv6").toggle(function(){
...
// level-6Comment-Submit

/* $("#<portlet:namespace/>responseDiscussionInput6").keyup(function(){
	if($.trim($("#<portlet:namespace/>responseDiscussionInput6").val()) !== '') {
		$("#<portlet:namespace/>level-6Comment-Submit").removeClass("np-display-off");
	} else {
		$("#<portlet:namespace/>level-6Comment").addClass("np-display-off");
