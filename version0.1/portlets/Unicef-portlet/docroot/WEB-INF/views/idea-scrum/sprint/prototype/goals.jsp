...
<div class="np-rounded-box clearfix"><div class="chat-container clearfix np-rounded-box">
	<div class="chat-question-box clearfix">
		<div class="chat-user-img-box clearfix">
		    ...
		    <div class="chat-user-img-text03">${inventorName}</div>
	    </div>
	    <div class="chat-user-comment">${sprint3.text}
	    	<p class="sprint-discussion-title-description">${sprint3TitleDescription}</p>
		    <div class="chat-user-reply-section clearfix">
		    	<div class="fr">
		    		<c:if test="${loggedInUserId == inventorId}">
			    		<a href="#" class="chat-user-tnx_btn">Thanks</a>
		    		</c:if>
		    			<a href="javascript:void(0);" class="chat-user-reply_btn" onclick="javascript:<portlet:namespace />postReplay3(0,0,0);">Reply</a>
		    	</div>
			</div>
		</div>
		<div class="chat-video-help "><a class="various-3"  href="#video-button">video link</a></div>
  	</div>
  
     <div id="discussionWrapper3" class="chat-container2">
    ...
    <div id="<portlet:namespace/>replayDiv3" style="display: none;" class="clearfix chat-ans-text-box">
			 <div id='<portlet:namespace/>replay' class="chat-user-img-box clearfix">
    			...
    			<div class="chat-user-img-text03">${loggedInUserName}</div>
    		</div>
			<div id="<portlet:namespace />replayDiv3">
    		<div class="chat-user-comment" id="<portlet:namespace />messageButtonWrapper3">
  				<textarea class="cs-chat-form-control-03" id="<portlet:namespace/>responseDiscussionInput3" name="<portlet:namespace/>responseDiscussionInput3"></textarea>
 				<a href="javascript:void(0);" id='<portlet:namespace/>level-3Comment' class="btn btn-info chat-btn">Reply</a>
 				<a href="javascript:void(0);" id='<portlet:namespace/>level-3Comment-Submit' class="btn btn-info chat-btn np-display-off" onclick="javascript:<portlet:namespace/>sendResponseMessage3();">Submit</a>
 			</div>
			</div>
	</div>
</div></div>
...
<div id="video-button" style="display:none;">
	<div class="responsive-container">
		<iframe allowfullscreen="" frameborder="0" id="player3" mozallowfullscreen="" src="http://player.vimeo.com/video/327329463?autoplay=false&amp;loop=false&amp;byline=false&amp;portrait=false&amp;title=false?autoplay=3" webkitallowfullscreen=""></iframe>
	</div>
</div>
...
$(document).ready(function() {
	...
	<portlet:namespace/>getAllDiscussions3();

	$("#<portlet:namespace/>responseDiscussionInput3").keypress(function (e) {
	        
			if($.trim($("#<portlet:namespace/>responseDiscussionInput3").val()) !== '') {
				$("#<portlet:namespace/>level-3Comment-Submit").removeClass("np-display-off");
			} else {
				$("#<portlet:namespace/>level-3Comment").addClass("np-display-off");
			}
			
			if($.trim($("#<portlet:namespace/>responseDiscussionInput3").val()) === '') { 
				$("#<portlet:namespace/>level-3Comment-Submit").addClass("np-display-off");
				$("#<portlet:namespace/>level-3Comment").removeClass("np-display-off");
			}
		
			var code = (e.keyCode ? e.keyCode : e.which);
	        if (code == 33 && !e.shiftKey) {
	           $("a#<portlet:namespace/>level-3Comment").click();
	        }else{
				return;
			}
	    });
...
<portlet:namespace />autoEnterKey3 = function(ideaScrumDiscussionId){
	$("#<portlet:namespace/>responseDiscussionInput3_"+ideaScrumDiscussionId).keypress(function (e) {
		
		if($.trim($("#<portlet:namespace/>responseDiscussionInput3_"+ideaScrumDiscussionId).val()) !== '') {
...
<portlet:namespace/>getAllDiscussions3 = function(){

	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint3.sprintId}";
	$.ajax({
	    ...
	    success: function( data ) {
			<portlet:namespace/>createDiscussionMessage3(data);
	    },
...
<portlet:namespace/>createDiscussionMessage3 = function(data){
	$("#discussionWrapper3").html(" ");
	var messageHTML = '';
	
	$.each(data, function(index, element) {
		
			messageHTML += '<div id="discussion_'+element.ideaScrumDiscussionId+'" class="Level_'+element.levelCount+'">';	
			messageHTML += <portlet:namespace />generateLevelHTML3(element.ideaScrumDiscussionId, element.avatarURL, element.userName,
			...
			if(element.isChild.toString() == 'true'){
				...
				$.each(element.secondLevelMessages, function(index, secondElement) {
						messageHTML += <portlet:namespace />generateLevelHTML3(secondElement.ideaScrumDiscussionId, secondElement.avatarURL, 
						...
						if(secondElement.isChild.toString() == 'true'){
							...
							$.each(secondElement.thirdLevelMessages, function(index, thirdElement) {
									messageHTML += <portlet:namespace />generateLevelHTML3(thirdElement.ideaScrumDiscussionId, thirdElement.avatarURL, 
			...
			$("#discussionWrapper3").append(messageHTML);
			messageHTML = '';
	});
}
<portlet:namespace />generateLevelHTML3 = function(ideaScrumDiscussionId, avatarURL, userName, message, parentDiscussionId, level, levelCount, isChild,userId,isThanks,isInventorPrefsAdmin,isLiked){
	var levelHTML = '';
	if(isChild == 'true'){
		levelHTML += '<div class="cab-leval-'+levelCount+'" id="cab-level-'+ideaScrumDiscussionId+'">';
						...
						levelHTML +='<div class="chat-user-img-box clearfix">' +
								...
								'</div>' +
								'<div class="chat-user-img-text03">'+userName+'</div>' +
							'</div>' + 
							'<div class="chat-user-ansr-comment"><textarea id="discussion-message_'+ideaScrumDiscussionId+'" class="discussion-message-text cs-chat-form-control-01" readonly="readonly">'+message+'</textarea>'+
								'<div class="chat-user-reply-section clearfix">' +
					            	'<div class="fr">';
					            		...
					            		if(isLiked === false ){
						            		levelHTML += '<a href="javascript:void(0);" class="like-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />deLikeDiscussion3('+ideaScrumDiscussionId+');" id="like-button_'+ideaScrumDiscussionId+'"></a>';
				            				levelHTML += '<a href="javascript:void(0);" class="liked-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />likeDiscussion3('+ideaScrumDiscussionId+');" id="liked-button_'+ideaScrumDiscussionId+'"></a>';
										} else {
											levelHTML += '<a href="javascript:void(0);" class="like-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />deLikeDiscussion3('+ideaScrumDiscussionId+');" id="like-button_'+ideaScrumDiscussionId+'"></a>';
				            				levelHTML += '<a href="javascript:void(0);" class="liked-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />likeDiscussion3('+ideaScrumDiscussionId+');" id="liked-button_'+ideaScrumDiscussionId+'"></a>';													
										}
					            	
					            		if(isInventorPrefsAdmin == "true") {
					            			if($("#<portlet:namespace/>isDiscussionInEditMode").hasClass("np-display-off")) {
					            				levelHTML += '<a href="javascript:void(0);" class="edit-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />editDiscussion3('+ideaScrumDiscussionId+');" id="edit-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="update-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />updateDiscussion3('+ideaScrumDiscussionId+');" id="update-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="cancel-update-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />cancelUpdateDiscussion3('+ideaScrumDiscussionId+');" id="cancel-update-button_'+ideaScrumDiscussionId+'"></a>';
					            			} else {
					            				levelHTML += '<a href="javascript:void(0);" class="edit-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />editDiscussion3('+ideaScrumDiscussionId+');" id="edit-button_'+ideaScrumDiscussionId+'"></a>';	
					            				levelHTML += '<a href="javascript:void(0);" class="update-discussion-board chat-user-tnx_btn np-display-off" onclick="javascript:<portlet:namespace />updateDiscussion3('+ideaScrumDiscussionId+');" id="update-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="cancel-update-discussion-board chat-user-tnx_btn np-display-off" onclick="javascript:<portlet:namespace />cancelUpdateDiscussion3('+ideaScrumDiscussionId+');" id="cancel-update-button_'+ideaScrumDiscussionId+'"></a>';
					            			}							            			
					            		}
					            		
					            		if(isThanks == "false" && isInventorPrefsAdmin == "true"){
					            			levelHTML += '<a href="javascript:void(0);" class="chat-user-tnx_btn" onclick="javascript:<portlet:namespace />discussionThanks3('+ideaScrumDiscussionId+');" id="thanks-button_'+ideaScrumDiscussionId+'">Thanks</a>';
										...
										if(levelCount != 3){
											...
											levelHTML += '<a href="javascript:void(0);" class="chat-user-reply_btn"' +
											                'onclick="javascript:<portlet:namespace />postReplay3('+parentDiscussionId+','+levelCount+','+ideaScrumDiscussionId+');">Reply</a>';
										}
							...
							'<textarea class="cs-chat-form-control-03" id="<portlet:namespace/>responseDiscussionInput3_'+ideaScrumDiscussionId+'" name="<portlet:namespace/>responseDiscussionInput3_'+ideaScrumDiscussionId+'"></textarea>'+
								...
								'onclick="javascript:<portlet:namespace/>sendResponseMessage3();">Submit</a>'+
						'</div>'+
					   '</div>';
		}else{
			levelHTML += '<div class="cab-leval-'+levelCount+'" id="cab-level-'+ideaScrumDiscussionId+'">' ;
...
<portlet:namespace/>deLikeDiscussion3 = function(ideaScrumDiscussionId){
...
<portlet:namespace/>likeDiscussion3 = function(ideaScrumDiscussionId){
	isPopUpEdit=false;
...
<portlet:namespace/>editDiscussion3 = function(ideaScrumDiscussionId){
...
<portlet:namespace/>cancelUpdateDiscussion3 = function(ideaScrumDiscussionId){ 
...
<portlet:namespace/>updateDiscussion3 = function(ideaScrumDiscussionId){ 
	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint3.sprintId}";
...
<portlet:namespace/>discussionThanks3 = function(ideaScrumDiscussionId){
	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint3.sprintId}";
	$.ajax({
...
<portlet:namespace/>sendResponseMessage3 = function(){
	var message = "";
	...
	if(level == 0){
		 message=$("#<portlet:namespace/>responseDiscussionInput3").val();
		  if($.trim(message) == ''){
			$('#<portlet:namespace />messageButtonWrapper3').addClass("has-error");
		  return;
		}
	}else{
		message =$('#<portlet:namespace/>responseDiscussionInput3_'+ideaScrumDiscussionId).val();
		  if($.trim(message) == ''){
	...
	$.ajax({
	    ...
	    success: function( data ) {
			...
			if(level == 0){
				 $('#<portlet:namespace/>responseDiscussionInput3').val('');
				 $('#<portlet:namespace/>responseDiscussionInput3').css('height','54px');
					<portlet:namespace/>goToLastMessage();
					$("#<portlet:namespace/>replayDiv3").toggle(function(){
						$("#<portlet:namespace/>replayDiv3").slideUp();
						isPopUpEdit = true;
					});
				$('#<portlet:namespace />messageButtonWrapper3').removeClass('has-error');
			}else if(level == 3){
				$('#<portlet:namespace/>responseDiscussionInput3_'+ideaScrumDiscussionId).val('');
				 $('#<portlet:namespace/>responseDiscussionInput3_'+ideaScrumDiscussionId).css('height','54px');
...
<portlet:namespace/>goToLastMessage = function(){
	 $("#discussionWrapper3").animate({ scrollTop: $("#discussionWrapper3")[0].scrollHeight}, 0);
 }
 
<portlet:namespace />postReplay3 = function(parentDiscussionId, level, ideaScrumDiscussionId){
	 autosize(document.querySelectorAll('textarea'));
	 <portlet:namespace />autoEnterKey3(ideaScrumDiscussionId);
	if(level == 0){
		...
		$("#<portlet:namespace/>responseDiscussionInput3" ).val('');
		if ($("#<portlet:namespace/>replayDiv3" ).css('display') == 'none') {
			$("#<portlet:namespace/>replayDiv3").toggle(function(){
				   $("#<portlet:namespace/>replayDiv3").slideDown();
				 });
				isPopUpEdit = false;
		}else{
		    $("#<portlet:namespace/>replayDiv3").toggle(function(){
...
// level-3Comment-Submit

/* $("#<portlet:namespace/>responseDiscussionInput3").keyup(function(){
	if($.trim($("#<portlet:namespace/>responseDiscussionInput3").val()) !== '') {
		$("#<portlet:namespace/>level-3Comment-Submit").removeClass("np-display-off");
	} else {
		$("#<portlet:namespace/>level-3Comment").addClass("np-display-off");
