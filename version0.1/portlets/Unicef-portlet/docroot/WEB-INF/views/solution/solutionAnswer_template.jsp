...
<%
	...
	/* String upvoteUser = StringPool.BLANK;
	String userList = (String)pageContext.getAttribute("userVoteList");
	
	List<String>upvoteUserList = Arrays.asList(userList.split(","));
	if(upvoteUserList.size() > 5){
		List<String>displayUserList = upvoteUserList.subList(0, 5);
		for(String voteUser : displayUserList){
			upvoteUser = upvoteUser + voteUser + ",";	   
		}
		upvoteUser = upvoteUser + ".";
	}else{
		upvoteUser = userList;
	}
	pageContext.setAttribute("upvoteUsers", upvoteUser); */
%>
 ...
 <div class="clearfix np-bor-03" id="solutionAns_${amswer.answerId}">
        ...
        <div class="col-sm-11 col-xs-12 ">
          ...
          <div style="display: none;" id='<portlet:namespace/>answerCommentDiv_${amswer.answerId}' class="col-sm-11 col-sm-offset-1 np-bor-02 buffer-top col-xs-12"> 
	          <c:if test="<%=themeDisplay.isSignedIn() %>">
	         	 <div class="col-xs-12 clearfix buffer-bottom" id='<portlet:namespace/>cmtDiv_${amswer.answerId}'>
	    			<div class="np-title02">Post Your Comments</div>
	    				<textarea id='<portlet:namespace/>solutionAnswerCommentTxt_${amswer.answerId}' placeholder="Place your comment here.." rows="1" class="form-control" style="width: 100%;"></textarea>
	    				<div class="pull-right buffer-top">
	    			 	<a href="javascript:void(0);" class="btn btn-primary" onclick="javascript:<portlet:namespace />postAnswerComment(${amswer.answerId});">
