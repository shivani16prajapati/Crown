...
<portlet:resourceURL var="solutionLikeURL" id="solutionLikeURL">
	<portlet:param name="solution" value="like"/>
</portlet:resourceURL> 
...
<portlet:namespace />solutionAnswerVote = function(solutionAnswerId){
 ...
 <portlet:namespace />solutionLike = function(solutionId){
	var like = $('#<portlet:namespace />likeUnlikeTextSpan').text();
	$.ajax({
		   url :"<%=solutionLikeURL.toString()%>",
				data : {
					like : like,
					solutionId : solutionId,
				},
				success : function(data) {
				$('#<portlet:namespace />solutionLikeSpan').text(data);
					
				
				if(like == "UpVote"){
					$('#<portlet:namespace />likeUnlikeTextSpan').text('DownVote');	
				}else if(like == "DownVote"){
					$('#<portlet:namespace />likeUnlikeTextSpan').text('UpVote');
				}
				}
			});
	} 
$(document).ready(function () {
	 ...
	 $("#<portlet:namespace/>solutionAnswerTxt").keypress(function (e) {
        var code = (e.keyCode ? e.keyCode : e.which);
        if (code == 13 && !e.shiftKey) {
           $("a#<portlet:namespace />answerClick").click();
        }else{
			return;
		}
    });
...
<portlet:namespace />postSolutionComment = function(solutionId){
	 ...
	 if($.trim(solutionCommentText) == ''){
	  $('#<portlet:namespace/>comment').addClass("has-error");
	  return;
	 }
	 $.ajax({
...
<portlet:namespace />postAnswer = function(solutionId){
	 var solutionAnswerText = $('#<portlet:namespace/>solutionAnswerTxt').val();
	 if($.trim(solutionAnswerText) == ''){
	  $('#<portlet:namespace/>answerDiv').addClass("has-error");
	  return;
	 }
	 $.ajax({
	    url :"<%=postSolutionAnswerUrl%>",
	    data : {
	    'solutionId' : solutionId,
	    'commentText' : solutionAnswerText,
	    },
	    success : function(result) {
	     $('#<portlet:namespace/>solutionAnswerTxt').val('');
	     $('#<portlet:namespace/>solutionAnswerTxt').val('');
	     $('#<portlet:namespace/>answerDiv').removeClass("has-error");
		 $('#<portlet:namespace/>solutionAnswerTxt').css('height','50px');
	     displaySolutionAnswerView(solutionId, 'postAns');
	   }
	 });
	}
