...
SearchContext folderSearchContext = SearchContextFactory.getInstance(request);
%>
...
<div class="<%= cssClass %>" data-facetFieldName="<%= facet.getFieldId() %>" id="<%= randomNamespace %>facet">
	...
	<ul class="folders nav nav-pills nav-stacked">
		...
		for (int i = 0; i < termCollectors.size(); i++) {
			...
			folderSearchContext.setFolderIds(new long[] {curFolderId});
			folderSearchContext.setKeywords(StringPool.BLANK);

			Hits results = indexer.search(folderSearchContext);
