<%--
/**
 * Copyright (c) 2000-2013 Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */
--%>

<%@ include file="/html/portlet/login/init.jsp" %>

<liferay-util:buffer var="navigation">
	<liferay-ui:icon-list>

		<%
		for (String section : PropsValues.LOGIN_FORM_NAVIGATION_PRE) {
			if(_getSectionJsp(section).equals("open_id")){
				continue;
			}else if(_getSectionJsp(section).equals("create_account")){
				continue;
			}
			
		%>

			<liferay-util:include page='<%= "/html/portlet/login/navigation/" + _getSectionJsp(section) + ".jsp" %>' portletId="<%= portletDisplay.getRootPortletId() %>" />

		<%
		}

		for (String section : PropsValues.LOGIN_FORM_NAVIGATION_POST) {
			if(_getSectionJsp(section).equals("open_id")){
				continue;
			}else if(_getSectionJsp(section).equals("create_account")){
				continue;
			}
		%>

			<liferay-util:include page='<%= "/html/portlet/login/navigation/" + _getSectionJsp(section) + ".jsp" %>' portletId="<%= portletDisplay.getRootPortletId() %>" />

		<%
		}
		%>

	</liferay-ui:icon-list>
</liferay-util:buffer>

<%
navigation = navigation.trim();
%>

<c:if test="<%= Validator.isNotNull(navigation) %>">
	<div class="np-forgot-link np-forgot-iconfix01">
		<%= navigation %>
	</div>
</c:if>

<%!
private String _getSectionJsp(String name) {
	return TextFormatter.format(name, TextFormatter.N);
}
%>


<style>

/* .np-forgot-iconfix01{ font-size:13px !important; line-height:25px; padding-top:4px;}
.np-forgot-iconfix01 .taglib-icon img{ display:none; }
.np-forgot-iconfix01 a{ color:#fff; padding-top:2px; font-weight:bold;}
.np-forgot-iconfix01 a:hover{ color:#fff; text-decoration:underline;}
.np-forgot-iconfix01 .taglib-icon-list li{ margin-top:-7px;}
 */
</style>


