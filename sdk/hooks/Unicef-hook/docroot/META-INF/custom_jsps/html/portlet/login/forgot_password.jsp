	...
	<aui:fieldset>
		<c:choose>
			<c:when test="<%= user2 == null %>">
                ...
                <div class="np-input">  
					<aui:input label="<%= loginLabel %>" name="<%= loginParameter %>" size="30" type="text" value="<%= loginValue %>" cssClass="np-form-control">
				...
				<div class="np-input-button">
					<aui:button type="submit" value='<%= PropsValues.USERS_REMINDER_QUERIES_ENABLED ? "next" : "send-new-password" %>' cssClass="np-submit"  />
				</div>
			...
			<c:when test="<%= (user2 != null) && Validator.isNotNull(user2.getEmailAddress()) %>">
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<div class="np-input-button">
						   <aui:button type="submit" value='<%= company.isSendPasswordResetLink() ? "send-password-reset-link" : "send-new-password" %>' cssClass="np-submit"/>
						</div>   
					</c:otherwise>
				</c:choose>
			</c:when>
...
<style>
/* .main-logo{
	margin: 140px auto 15px auto !important;
}
.control-label{
