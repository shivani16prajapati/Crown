...
<div class="np-rounded-box clearfix"><div class="chat-container clearfix np-rounded-box">
	<div class="chat-question-box clearfix">
		<div class="chat-user-img-box clearfix">
		    ...
		    <div class="chat-user-img-text04">${inventorName}</div>
	    </div>
	    <div class="chat-user-comment">${sprint4.text}
	    	<p class="sprint-discussion-title-description">${sprint4TitleDescription}</p>
		    <div class="chat-user-reply-section clearfix">
		    	<div class="fr">
		    		<c:if test="${loggedInUserId == inventorId}">
			    		<a href="#" class="chat-user-tnx_btn">Thanks</a>
		    		</c:if>
		    			<a href="javascript:void(0);" class="chat-user-reply_btn" onclick="javascript:<portlet:namespace />postReplay4(0,0,0);">Reply</a>
		    	</div>
			</div>
		</div>
		<div class="chat-video-help "><a class="various-4"  href="#video-button">video link</a></div>
  	</div>
  
     <div id="discussionWrapper4" class="chat-container2">
    ...
    <div id="<portlet:namespace/>replayDiv4" style="display: none;" class="clearfix chat-ans-text-box">
			 <div id='<portlet:namespace/>replay' class="chat-user-img-box clearfix">
    			...
    			<div class="chat-user-img-text04">${loggedInUserName}</div>
    		</div>
			<div id="<portlet:namespace />replayDiv4">
    		<div class="chat-user-comment" id="<portlet:namespace />messageButtonWrapper4">
  				<textarea class="cs-chat-form-control-04" id="<portlet:namespace/>responseDiscussionInput4" name="<portlet:namespace/>responseDiscussionInput4"></textarea>
 				<a href="javascript:void(0);" id='<portlet:namespace/>level-4Comment' class="btn btn-info chat-btn">Reply</a>
 				<a href="javascript:void(0);" id='<portlet:namespace/>level-4Comment-Submit' class="btn btn-info chat-btn np-display-off" onclick="javascript:<portlet:namespace/>sendResponseMessage4();">Submit</a>
 			</div>
			</div>
	</div>
</div></div>
...
<div id="video-button" style="display:none;">
	<div class="responsive-container">
		<iframe allowfullscreen="" frameborder="0" id="player4" mozallowfullscreen="" src="http://player.vimeo.com/video/427329463?autoplay=false&amp;loop=false&amp;byline=false&amp;portrait=false&amp;title=false?autoplay=4" webkitallowfullscreen=""></iframe>
	</div>
</div>
...
$(document).ready(function() {
	...
	<portlet:namespace/>getAllDiscussions4();

	$("#<portlet:namespace/>responseDiscussionInput4").keypress(function (e) {
	        
			if($.trim($("#<portlet:namespace/>responseDiscussionInput4").val()) !== '') {
				$("#<portlet:namespace/>level-4Comment-Submit").removeClass("np-display-off");
			} else {
				$("#<portlet:namespace/>level-4Comment").addClass("np-display-off");
			}
			
			if($.trim($("#<portlet:namespace/>responseDiscussionInput4").val()) === '') { 
				$("#<portlet:namespace/>level-4Comment-Submit").addClass("np-display-off");
				$("#<portlet:namespace/>level-4Comment").removeClass("np-display-off");
			}
		
			var code = (e.keyCode ? e.keyCode : e.which);
	        if (code == 43 && !e.shiftKey) {
	           $("a#<portlet:namespace/>level-4Comment").click();
	        }else{
				return;
			}
	    });
...
<portlet:namespace />autoEnterKey4 = function(ideaScrumDiscussionId){
	$("#<portlet:namespace/>responseDiscussionInput4_"+ideaScrumDiscussionId).keypress(function (e) {
		
		if($.trim($("#<portlet:namespace/>responseDiscussionInput4_"+ideaScrumDiscussionId).val()) !== '') {
...
<portlet:namespace/>getAllDiscussions4 = function(){

	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint4.sprintId}";
	$.ajax({
	    ...
	    success: function( data ) {
			<portlet:namespace/>createDiscussionMessage4(data);
	    },
...
<portlet:namespace/>createDiscussionMessage4 = function(data){
	$("#discussionWrapper4").html(" ");
	var messageHTML = '';
	
	$.each(data, function(index, element) {
		
			messageHTML += '<div id="discussion_'+element.ideaScrumDiscussionId+'" class="Level_'+element.levelCount+'">';	
			messageHTML += <portlet:namespace />generateLevelHTML4(element.ideaScrumDiscussionId, element.avatarURL, element.userName,
			...
			if(element.isChild.toString() == 'true'){
				...
				$.each(element.secondLevelMessages, function(index, secondElement) {
						messageHTML += <portlet:namespace />generateLevelHTML4(secondElement.ideaScrumDiscussionId, secondElement.avatarURL, 
						...
						if(secondElement.isChild.toString() == 'true'){
							...
							$.each(secondElement.thirdLevelMessages, function(index, thirdElement) {
									messageHTML += <portlet:namespace />generateLevelHTML4(thirdElement.ideaScrumDiscussionId, thirdElement.avatarURL, 
			...
			$("#discussionWrapper4").append(messageHTML);
			messageHTML = '';
	});
}
<portlet:namespace />generateLevelHTML4 = function(ideaScrumDiscussionId, avatarURL, userName, message, parentDiscussionId, level, levelCount, isChild,userId,isThanks,isInventorPrefsAdmin,isLiked){
	var levelHTML = '';
	if(isChild == 'true'){
		levelHTML += '<div class="cab-leval-'+levelCount+'" id="cab-level-'+ideaScrumDiscussionId+'">';
						...
						levelHTML +='<div class="chat-user-img-box clearfix">' +
								...
								'</div>' +
								'<div class="chat-user-img-text04">'+userName+'</div>' +
							'</div>' + 
							'<div class="chat-user-ansr-comment"><textarea id="discussion-message_'+ideaScrumDiscussionId+'" class="discussion-message-text cs-chat-form-control-01" readonly="readonly">'+message+'</textarea>'+
								'<div class="chat-user-reply-section clearfix">' +
					            	'<div class="fr">';
					            		...
					            		if(isLiked === false ){
						            		levelHTML += '<a href="javascript:void(0);" class="like-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />deLikeDiscussion4('+ideaScrumDiscussionId+');" id="like-button_'+ideaScrumDiscussionId+'"></a>';
				            				levelHTML += '<a href="javascript:void(0);" class="liked-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />likeDiscussion4('+ideaScrumDiscussionId+');" id="liked-button_'+ideaScrumDiscussionId+'"></a>';
										} else {
											levelHTML += '<a href="javascript:void(0);" class="like-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />deLikeDiscussion4('+ideaScrumDiscussionId+');" id="like-button_'+ideaScrumDiscussionId+'"></a>';
				            				levelHTML += '<a href="javascript:void(0);" class="liked-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />likeDiscussion4('+ideaScrumDiscussionId+');" id="liked-button_'+ideaScrumDiscussionId+'"></a>';													
										}
					            	
					            		if(isInventorPrefsAdmin == "true") {
					            			if($("#<portlet:namespace/>isDiscussionInEditMode").hasClass("np-display-off")) {
					            				levelHTML += '<a href="javascript:void(0);" class="edit-discussion-board np-display-off chat-user-tnx_btn" onclick="javascript:<portlet:namespace />editDiscussion4('+ideaScrumDiscussionId+');" id="edit-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="update-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />updateDiscussion4('+ideaScrumDiscussionId+');" id="update-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="cancel-update-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />cancelUpdateDiscussion4('+ideaScrumDiscussionId+');" id="cancel-update-button_'+ideaScrumDiscussionId+'"></a>';
					            			} else {
					            				levelHTML += '<a href="javascript:void(0);" class="edit-discussion-board chat-user-tnx_btn" onclick="javascript:<portlet:namespace />editDiscussion4('+ideaScrumDiscussionId+');" id="edit-button_'+ideaScrumDiscussionId+'"></a>';	
					            				levelHTML += '<a href="javascript:void(0);" class="update-discussion-board chat-user-tnx_btn np-display-off" onclick="javascript:<portlet:namespace />updateDiscussion4('+ideaScrumDiscussionId+');" id="update-button_'+ideaScrumDiscussionId+'"></a>';
					            				levelHTML += '<a href="javascript:void(0);" class="cancel-update-discussion-board chat-user-tnx_btn np-display-off" onclick="javascript:<portlet:namespace />cancelUpdateDiscussion4('+ideaScrumDiscussionId+');" id="cancel-update-button_'+ideaScrumDiscussionId+'"></a>';
					            			}							            			
					            		}
					            		
					            		if(isThanks == "false" && isInventorPrefsAdmin == "true"){
					            			levelHTML += '<a href="javascript:void(0);" class="chat-user-tnx_btn" onclick="javascript:<portlet:namespace />discussionThanks4('+ideaScrumDiscussionId+');" id="thanks-button_'+ideaScrumDiscussionId+'">Thanks</a>';
										...
										if(levelCount != 3){
											...
											levelHTML += '<a href="javascript:void(0);" class="chat-user-reply_btn"' +
											                'onclick="javascript:<portlet:namespace />postReplay4('+parentDiscussionId+','+levelCount+','+ideaScrumDiscussionId+');">Reply</a>';
										}
							...
							'<textarea class="cs-chat-form-control-04" id="<portlet:namespace/>responseDiscussionInput4_'+ideaScrumDiscussionId+'" name="<portlet:namespace/>responseDiscussionInput4_'+ideaScrumDiscussionId+'"></textarea>'+
								...
								'onclick="javascript:<portlet:namespace/>sendResponseMessage4();">Submit</a>'+
						'</div>'+
					   '</div>';
		}else{
			levelHTML += '<div class="cab-leval-'+levelCount+'" id="cab-level-'+ideaScrumDiscussionId+'">' ;
...
<portlet:namespace/>deLikeDiscussion4 = function(ideaScrumDiscussionId){
...
<portlet:namespace/>likeDiscussion4 = function(ideaScrumDiscussionId){
	isPopUpEdit=false;
...
<portlet:namespace/>editDiscussion4 = function(ideaScrumDiscussionId){
...
<portlet:namespace/>cancelUpdateDiscussion4 = function(ideaScrumDiscussionId){ 
...
<portlet:namespace/>updateDiscussion4 = function(ideaScrumDiscussionId){ 
	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint4.sprintId}";
...
<portlet:namespace/>discussionThanks4 = function(ideaScrumDiscussionId){
	var ideaScrumId = "${ideaScrumId}";
	var ideaId = "${ideaId}";
	var sprintId = "${sprint4.sprintId}";
	$.ajax({
...
<portlet:namespace/>sendResponseMessage4 = function(){
	var message = "";
	...
	if(level == 0){
		 message=$("#<portlet:namespace/>responseDiscussionInput4").val();
		  if($.trim(message) == ''){
			$('#<portlet:namespace />messageButtonWrapper4').addClass("has-error");
		  return;
		}
	}else{
		message =$('#<portlet:namespace/>responseDiscussionInput4_'+ideaScrumDiscussionId).val();
		  if($.trim(message) == ''){
	...
	$.ajax({
	    ...
	    success: function( data ) {
			...
			if(level == 0){
				 $('#<portlet:namespace/>responseDiscussionInput4').val('');
				 $('#<portlet:namespace/>responseDiscussionInput4').css('height','54px');
					<portlet:namespace/>goToLastMessage();
					$("#<portlet:namespace/>replayDiv4").toggle(function(){
						$("#<portlet:namespace/>replayDiv4").slideUp();
						isPopUpEdit = true;
					});
				$('#<portlet:namespace />messageButtonWrapper4').removeClass('has-error');
			}else if(level == 4){
				$('#<portlet:namespace/>responseDiscussionInput4_'+ideaScrumDiscussionId).val('');
				 $('#<portlet:namespace/>responseDiscussionInput4_'+ideaScrumDiscussionId).css('height','54px');
...
<portlet:namespace/>goToLastMessage = function(){
	 $("#discussionWrapper4").animate({ scrollTop: $("#discussionWrapper4")[0].scrollHeight}, 0);
 }
 
<portlet:namespace />postReplay4 = function(parentDiscussionId, level, ideaScrumDiscussionId){
	 autosize(document.querySelectorAll('textarea'));
	 <portlet:namespace />autoEnterKey4(ideaScrumDiscussionId);
	if(level == 0){
		...
		$("#<portlet:namespace/>responseDiscussionInput4" ).val('');
		if ($("#<portlet:namespace/>replayDiv4" ).css('display') == 'none') {
			$("#<portlet:namespace/>replayDiv4").toggle(function(){
				   $("#<portlet:namespace/>replayDiv4").slideDown();
				 });
				isPopUpEdit = false;
		}else{
		    $("#<portlet:namespace/>replayDiv4").toggle(function(){
...
// level-4Comment-Submit

/* $("#<portlet:namespace/>responseDiscussionInput4").keyup(function(){
	if($.trim($("#<portlet:namespace/>responseDiscussionInput4").val()) !== '') {
		$("#<portlet:namespace/>level-4Comment-Submit").removeClass("np-display-off");
	} else {
		$("#<portlet:namespace/>level-4Comment").addClass("np-display-off");
